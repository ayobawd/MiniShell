...
int ms_execute_line(t_cmd *pipeline, t_env **env)
{
    ...
    if (!single->next && single->argv && single->argv[0]
        ...
        && ((!ft_strncmp(single->argv[0], "cd", 2) && single->argv[0][2] == '\0')
            || (!ft_strncmp(single->argv[0], "export", 6) && single->argv[0][6] == '\0')
            || (!ft_strncmp(single->argv[0], "unset", 5) && single->argv[0][5] == '\0')
            || (!ft_strncmp(single->argv[0], "exit", 4) && single->argv[0][4] == '\0')))
    {
