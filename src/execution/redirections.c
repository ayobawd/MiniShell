...
int	setup_redirections(t_cmds *cmd)
{
	...
	while (i < cmd->red_len)
	{
		...
		{
			fd = open(cmd->outs[i].file_name, O_WRONLY | O_CREAT | O_TRUNC, 0644);
			if (fd == -1)
			{
				perror(cmd->outs[i].file_name);
				return (-1);
		...
		{
			fd = open(cmd->outs[i].file_name, O_WRONLY | O_CREAT | O_APPEND, 0644);
			if (fd == -1)
			{
				perror(cmd->outs[i].file_name);
				return (-1);
		...
		{
			if (setup_heredoc(cmd->outs[i].file_name) == -1)
				return (-1);
		}
		i++;
	}
